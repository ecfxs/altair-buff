...
# 再开一个终端：造 3 台假设备，让面板有数据（仅开发/CI 用）
make fake
```
...
> **假设备（`fake-device`）只用于开发与验收，绝不进生产**：
> 它带硬护栏 —— 目标不是 `127.0.0.1`/`localhost` 时直接拒绝（除非显式 `--force`）。
> 理由很实际：假设备一旦打到生产服务器，面板里会混进 `fake01/fake02` 把真实设备淹掉，
> 它们还会被下发配置、占存储。生产环境部署后不要运行这个命令。
